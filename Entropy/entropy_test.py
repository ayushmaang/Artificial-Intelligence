...
for i in range(10,1002,1):
    ...
    for j in range(10):
        ...
        if i == 200:
            afpiaejf = "hi"
        DS, headers, rows = get_data("quizD_train.csv",0,i)
        actualData = answer("quizD_test_a.csv")
        ...
        treeSize = 0
        correct = 0
        answers = test_data(headNode, "quizD_test_a.csv", 0, 1000)
        skipped = 0
        ...
        average += current
        if j == 0:
            print (str(current) + " " + str(average))
        averageCount+=1
    average/=10

    arrayy.append(average)
    arrayx.append(i)
